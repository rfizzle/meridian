...
| Remaining shelf blocks (≈24 textures) | **MISSING** | `assets/meridian/textures/block/*.png` — `rectifier*`, sight + treasure shelves, and the custom-model `library/`, `ender_library/`, `filtering_shelf/` sets. Sourced from Zenith (orig. Apotheosis) per README attribution — not yet glyph-authored |
