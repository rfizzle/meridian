...
| Remaining shelf blocks (≈14 textures) | **MISSING** | `assets/meridian/textures/block/*.png` — the custom-model `library/`, `ender_library/`, `filtering_shelf/` sets (multi-face Blockbench models). Sourced from Zenith (orig. Apotheosis) per README attribution — not yet glyph-authored |
