...
public class HarpoonConfigGameTest implements FabricGameTest {
    ...
    public void harpoonAffectsPlayersWhenConfigEnabled(GameTestHelper helper) {
        ...
        String enabledConfig = "{\n"
                + "  \"configVersion\": 1,\n"
                + "  \"combat\": { \"harpoonAffectsPlayers\": true }\n"
                + "}";
        try {
            Files.writeString(CONFIG_FILE, enabledConfig);
            Meridian.reloadConfig(helper.getLevel().getServer());
        } catch (IOException e) {
            Meridian.LOGGER.error("Could not write config in harpoonAffectsPlayersWhenConfigEnabled", e);
        ...
        try {
            if (!ProjectileEnchantmentHandler.harpoonVictimAllowed(player)) {
                helper.fail("Players should be eligible Harpoon victims with combat.harpoonAffectsPlayers=true");
                return;
            }
            helper.succeed();
        } finally {
