...
/**
 * Covers the {@code combat.staggerAffectsPlayers} toggle's enabled path (the disabled
 ...
 */
public class StaggerConfigGameTest implements FabricGameTest {

    private static final Path CONFIG_FILE =
            FabricLoader.getInstance().getConfigDir().resolve("meridian.json");

    @GameTest(template = "meridian:empty_3x3", batch = "meridianConfigMutation13")
    public void staggerAffectsPlayersWhenConfigEnabled(GameTestHelper helper) {
        byte[] original;
        try {
        ...
        String enabledConfig = "{\n"
                + "  \"configVersion\": 1,\n"
                + "  \"combat\": { \"staggerAffectsPlayers\": true }\n"
                + "}";
        try {
            Files.writeString(CONFIG_FILE, enabledConfig);
            Meridian.reloadConfig(helper.getLevel().getServer());
        } catch (IOException e) {
            Meridian.LOGGER.error("Could not write config in staggerAffectsPlayersWhenConfigEnabled", e);
        ...
        try {
            if (!EnchantmentEffectHandler.staggerTargetAllowed(player)) {
                helper.fail("Players should be eligible Stagger targets with combat.staggerAffectsPlayers=true");
                return;
            }
            helper.succeed();
        } finally {
