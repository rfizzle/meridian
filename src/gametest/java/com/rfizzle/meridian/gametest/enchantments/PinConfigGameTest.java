...
/**
 * Covers the {@code combat.pinAffectsPlayers} toggle's enabled path (the disabled default is
 * asserted in {@code RangedEnchantmentGameTest}). Rewrites the shared {@code config/meridian.json},
 * so it follows {@code UndertowConfigGameTest}'s rule: a unique {@code batch} serializes it against
 * the other config-mutating tests.
 */
public class PinConfigGameTest implements FabricGameTest {
    ...
    public void pinAffectsPlayersWhenConfigEnabled(GameTestHelper helper) {
        ...
        String enabledConfig = "{\n"
                + "  \"configVersion\": 1,\n"
                + "  \"combat\": { \"pinAffectsPlayers\": true }\n"
                + "}";
        try {
            Files.writeString(CONFIG_FILE, enabledConfig);
            Meridian.reloadConfig(helper.getLevel().getServer());
        } catch (IOException e) {
            Meridian.LOGGER.error("Could not write config in pinAffectsPlayersWhenConfigEnabled", e);
        ...
        try {
            if (!ProjectileEnchantmentHandler.pinVictimAllowed(player)) {
                helper.fail("Players should be eligible Pin victims with combat.pinAffectsPlayers=true");
                return;
            }
            helper.succeed();
        } finally {
