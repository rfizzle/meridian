...
/**
 * Covers the {@code combat.undertowAffectsPlayers} toggle's enabled path (the disabled
 * default is asserted in {@code RangedEnchantmentGameTest}). Rewrites the shared
 * {@code config/meridian.json}, so it follows {@code HarpoonConfigGameTest}'s rule: a
 * unique {@code batch} serializes it against the other config-mutating tests.
 */
public class UndertowConfigGameTest implements FabricGameTest {
    ...
    public void undertowAffectsPlayersWhenConfigEnabled(GameTestHelper helper) {
        ...
        String enabledConfig = "{\n"
                + "  \"configVersion\": 1,\n"
                + "  \"combat\": { \"undertowAffectsPlayers\": true }\n"
                + "}";
        try {
            Files.writeString(CONFIG_FILE, enabledConfig);
            Meridian.reloadConfig(helper.getLevel().getServer());
        } catch (IOException e) {
            Meridian.LOGGER.error("Could not write config in undertowAffectsPlayersWhenConfigEnabled", e);
        ...
        try {
            if (!ProjectileEnchantmentHandler.undertowVictimAllowed(player)) {
                helper.fail("Players should be eligible Undertow victims with combat.undertowAffectsPlayers=true");
                return;
            }
            helper.succeed();
        } finally {
