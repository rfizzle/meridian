...
/**
 * Covers the {@code combat.markAffectsPlayers} toggle's enabled path (the disabled
 ...
 */
public class MarkConfigGameTest implements FabricGameTest {
    ...
    public void markAffectsPlayersWhenConfigEnabled(GameTestHelper helper) {
        ...
        String enabledConfig = "{\n"
                + "  \"configVersion\": 1,\n"
                + "  \"combat\": { \"markAffectsPlayers\": true }\n"
                + "}";
        try {
            Files.writeString(CONFIG_FILE, enabledConfig);
            Meridian.reloadConfig(helper.getLevel().getServer());
        } catch (IOException e) {
            Meridian.LOGGER.error("Could not write config in markAffectsPlayersWhenConfigEnabled", e);
        ...
        try {
            if (!ProjectileEnchantmentHandler.markVictimAllowed(player)) {
                helper.fail("Players should be markable with combat.markAffectsPlayers=true");
                return;
            }
            helper.succeed();
        } finally {
