...
/**
 * One-stop registration hook for every {@link AnvilHandler}. Called from
 ...
 * non-empty result wins. Prismatic Web and iron-block anvil repair key off
 * disjoint right-slot items (web vs. iron block), so their relative order is behaviour-neutral.
 * The tome families slot in below them. The Extraction Tome has two handlers keyed on the
